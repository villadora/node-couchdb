...
RequestBase.prototype._request = function(url, options, callback) {
  var request = this.request || (this.request = require('request').defaults(this.options)),
    opt = {
      url: url
    };
  ...
  if (typeof opt.strictSSL == 'boolean')
    opt.rejectUnauthorized = opt.strictSSL;

  // set headers
  opt.headers = (opt.headers && typeof(opt.headers) == 'object') ? opt.headers : {};
  // as request.defauts don't support nested properties, copy headers here
  opt.headers = _.extend(opt.headers, this.options.headers || {});
