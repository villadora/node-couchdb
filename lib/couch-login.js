...
function makeReq(meth, body, f) {
  return function madeReq(p, d, cb) {
    ...
    var h = {}, u = url.resolve(this.couch, p),
      req = {
        uri: u,
        headers: h,
        json: true,
...
function changePass(auth, cb) {
  ...
  this.get(u, function(er, res, data) {
    ...
    Object.keys(auth).filter(function(k) {
      return k.charAt(0) !== '_' && k !== 'password' && k !== 'password_sha' && k !== 'salt'
    }).forEach(function(k) {
      data[k] = auth[k]
    })
...
function signup(auth, cb) {
  ...
  var newSalt = crypto.randomBytes(30).toString('hex'),
    ...
    user = {
      _id: 'org.couchdb.user:' + auth.name,
      name: auth.name,
      roles: [],
      type: 'user',
      password_sha: newSha,
...
function addToken(res) {
  ...
  sc = sc.split(/\s*;\s*/).map(function(p) {
    ...
    var k = p[0] === 'AuthSession' ? p[0] : p[0].toLowerCase(),
      v = k === 'expires' ? Date.parse(p[1]) : p[1] === '' || p[1] === undefined ? true // HttpOnly
      : p[1]
      set[k] = v
    return set
  }, {})
...
function logout(cb) {
  ...
  var h = {
    cookie: 'AuthSession=' + this.token.AuthSession
