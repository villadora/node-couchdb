...
_.extend(CouchDB.prototype, {
    ...
    dbUpdates: function(timeout, callback) {
        ...
        if (callback)
            this._get(url_module.resolve(this.url, '_db_updates') + '?' + qs.stringify(params), function(err, updates, res) {
    ...
    log: function(bytes, offset, callback) {
        ...
        this._get(url_module.resolve(this.url, '_log') + '?' + qs.stringify(query), function(err, body) {
            callback(err, body || '');
        });
    },
