...
_.extend(Database.prototype, {
    ...
    searchByKeys: function(startkey, endkey, options, callback) {
        ...
        if (!keys) {
            key && query.key(key);
            query.startkey(startkey || null).endkey(endkey || null).set(options).execute(callback);
        } else {
    ...
    searchByIds: function(startId, endId, options, callback) {
        ...
        this.select().betweenIds(startId || null, endId || null).set(options).execute(callback);
    },
    /**
     *
     */
