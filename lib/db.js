...
var RequestBase = require('./base');
var Executor = require('./exec');
...
function Database(url, dbname, options) {
  ...
  if (!url || !dbname)
    throw new Error('url, dbname must be provided');
...
_.extend(Database.prototype, {
  ...
  exists: function(callback) {
    this._head(this.url, function(err, body, res) {
      ...
      callback && callback(err, !err, body);
    });
  },
  ...
  /**
   * Get tempView result with given mapFn and reduceFn
   *
   * @param {function} mapFn map function for tempView
   * @param {function} reduceFn reduce function for tempView
   * @param {Object=} options
   * @param {function=} callback function called after response
   */
  tempView: function(mapFn, reduceFn, options, callback) {
  ...
  /**
   * Save a document to database, if doc have no revision, then create a new document.
   * @param {string} docid
   * @param {Object}  doc
   * @param {Object=} options
   * @param {function} callback
   */
  save: function(docid, doc, options, callback) {
    ...
    if (doc._rev) {
      this.doc(docid, options).set(doc).create(callback);
    } else
      this.doc(docid, options).set(doc).save(callback);
  },
  /**
   * @class
   * @name BulkSaveOption
   * @property {boolean=} new_edit
   */
  /**
   * Create new or update docs in bulk
   * @param {Array} docs
   * @param {BulkSaveOption} options
   * @param {function} callback
   */
  ...
  // Revisions
  // ==========================
  missingRevisions: function(query, callback) {
    this._post(this.url + '/_missing_revs', {
      body: JSON.stringify(query)
    }, function(err, body) {
      callback && callback(err, body);
    });
  },
  revisionsDiff: function(query, callback) {
