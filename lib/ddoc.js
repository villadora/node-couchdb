...
_.extend(DesignDoc.prototype, {
    ...
    // TODO: shows & lists
    // ======================
    show: function(showname, method) {
        var self = this;
        method = '_' + (method || 'GET').toLowerCase();
        return {
            get: function(options, callback) {
                ...
                var rs = self[method](self.dburl + '/' + self._id + '/_show/' + showname,
                    _.extend({}, options), callback);

                self.enableJson = true;
                return rs;
            },
            doc: function(id, options, callback) {
                ...
                self.enableJson = false;
                var rs = self[method](self.dburl + '/' + self._id + '/_show/' + showname + '/' + encodeURIComponent(id),
                    _.extend({}, options), callback);
                self.enableJson = true;
                return rs;
            }
    ...
    list: function(listname, method, callback) {
        var self = this;
        method = '_' + (method || 'GET').toLowerCase();

        return {
            view: function(viewname, options, callback) {
                if (arguments.length == 2 && typeof options == 'function') {
                    callback = options;
                    options = undefined;
                }
                ...
                self.enableJson = false;
                var rs = self[method](self.dburl + '/' + self._id + '/_list/' + listname + '/' + viewname,
                    _.extend({}, options), callback);
                self.enableJson = true;
                return rs;
            }
