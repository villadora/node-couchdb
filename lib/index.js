...
CouchDB.prototype.version = function(callback) {
    this._request(this.url, function(err, version, res) {
...
CouchDB.prototype.replicate = function(source, target, options, callback) {
    ...
    this._request(url_module.resolve(this.url, "/_replicate"), {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
            source: source,
...
CouchDB.prototype.newUuids = function(n, callback) {
    if (this.uuids_cache.length >= n) {
        ...
        this._request("/_uuids?count=" + (100 + n), 'GET', function(err, result) {
